...
const GROUP_IMAGES = [
  ["/assets/Osteopatia-v2.jpg", "/assets/Osteopatia-pediatrica-v2.jpg", "/assets/Fisioterapia-v4.jpg", "/assets/Massaggio-Decontratturante-Sportivo-v2.jpg"],
  ["/assets/Pilates.jpg", "/assets/Rinforzo-Posturale-v2.jpg", "/assets/Functional-Training-v3.jpg"],
  ["/assets/Massaggi-Rilassanti-v2.jpg", "/assets/Linfodrenante-v2.jpg", "/assets/Riflessologia-v2.jpg", "/assets/Coppettazione-v2.jpg"],
  ["/assets/Nutrizione-BIA-v3.jpg", "/assets/Mental-Coach-v2.jpg"],
];

type Service = { name: string; desc: string; msg: string };
type ServiceRendered = Service & { img: string };
type Group = { label: string; services: Service[] };
type GroupRendered = { label: string; services: ServiceRendered[] };

function HoverCard({ name, img, desc, cat, msg, prenotaLabel, prenotaWaLabel }: {
  name: string; img: string; desc: string; cat: string; msg: string; prenotaLabel: string; prenotaWaLabel: string;
}) {
...
function BenAccompagna() {
  ...
  const pillars = t.raw("accPillars") as { t: string; d: string }[];
  return (
    <section ref={ref as React.RefObject<HTMLElement>} className={`ben-acc${visible ? " is-on" : ""}`}>
      <div className="ben-acc__head">
        <span className="ben-acc__eyebrow ben-acc-rise ben-acc-rise--0">{t("accEyebrow")}</span>
        <h2 className="ben-acc__title ben-acc-rise ben-acc-rise--1">{t("accTitle")}</h2>
      ...
      <div className="ben-acc__row">
        {pillars.map((p, i) => (
          <div key={ACC_KEYS[i]} className={`ben-acc__pill ben-acc-rise ben-acc-rise--${3 + i}`}>
            <span className="ben-acc__k">{ACC_KEYS[i]}</span>
...
export const BenesserePageV2: React.FC = () => {
  ...
  const t = useTranslations("benessere");
  const groups: GroupRendered[] = (t.raw("groups") as Group[]).map((g, gi) => ({
    ...g,
    services: g.services.map((s, si) => ({ ...s, img: GROUP_IMAGES[gi][si] })),
  }));
  const features = t.raw("features") as { num: string; label: string; title: string; micro: string; tags: string[] }[];
            ...
            {groups.map((g) => (
              <div key={g.label}>
                ...
                <div className="vb-group__cards">
                  {g.services.map((s) => (
                    <HoverCard key={s.name} {...s} cat={g.label} prenotaLabel={t("prenotaCta")} prenotaWaLabel={t("prenotaWhatsapp")} />
                  ))}
                </div>
              </div>
            ))}
